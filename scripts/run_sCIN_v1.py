from sCIN.sCIN_v1 import train_sCIN_v1, get_embs_sCIN_v1
from configs import sCIN_v1
...
def main() -> None:
    ...
    rna_labels = rna_adata.obs["cell_type_encoded"].values
    atac_labels = atac_adata.obs["cell_type_encoded"].values
    ...
    for i, seed in enumerate(seeds):
        ...
        # rna_train, rna_test, rna_labels_train, rna_labels_test = train_test_split(rna_counts,
        #                                                                           rna_labels,
        #                                                                           test_size=0.3,
        #                                                                           random_state=seed)
        
        # atac_train, atac_test, atac_labels_train, atac_labels_test = train_test_split(atac_counts,
        #                                                                               atac_labels,
        #                                                                               test_size=0.3,
        #                                                                               random_state=seed)
        
        rna_train, rna_test, rna_labels_train, rna_labels_test, atac_train, atac_test, atac_labels_train, atac_labels_test = train_test_split(rna_counts,
                                                                                                                                              rna_labels,
                                                                                                                                              atac_counts,
                                                                                                                                              atac_labels,
                                                                                                                                              test_size=0.3,
        ...
        train_dict = train_sCIN_v1(rna_train, 
                                   atac_train,
                                   rna_labels_train,
                                   atac_labels_train,
                                   settings=sCIN_v1,
        ...
        rna_embs, atac_embs = get_embs_sCIN_v1(rna_test, 
                                               atac_test,
        ...
        rna_labels_df = pd.DataFrame(rna_labels_test)
        atac_labels_df = pd.DataFrame(atac_labels_test)
    ...
    results.to_csv(os.path.join(res_save_dir, f"metrics_sCINV1_{args.num_reps}reps.csv"), index=False)
