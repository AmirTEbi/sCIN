...
def plot_asw_5(df:pd.DataFrame, save_dir:str, legend_names:dict, mod_colors:dict, 
    ...
    legend_handles = [
        Line2D(
            [0], [0], color=mod_colors[modality], linewidth=2, 
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"fig5_b.{file_type}"))
...
def main() -> None:
    ...
    if args.unpaired:
        ...
        REP_DIRS = [f"rep{seed}" for seed in range(0, 
                                                   args.seed_range, 
                                                   10)]
        ...
        plot_asw_5(df=res, save_dir=PLOT_SAVE_DIR,legend_names=LEGENDS,
                   mod_colors=COLORS, xticks=XTICKS, file_type="pdf")
                   
        res.to_csv(os.path.join(UNPAIRED_MAIN_DIR, "unpaired_asw_mods.csv"), index=False)
