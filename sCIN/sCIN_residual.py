...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        return self.activation(self.linear(x) + self.skip(x))


class Mod1Encoder(nn.Module):
    """ An encoder with four layers"""
    def __init__(self, dims: List[int], latent_dim: int, dropout: float = 0.0):
        ...
        for in_d, out_d in zip(dims[:-1], dims[1:]):
            ...
            layers.append(ResidualBlock(in_dim=out_d, out_dim=out_d))
            if dropout > 0:
...
def train_sCIN(mod1_train: np.ndarray, 
    ...
    hidden_dim1 = settings.get("hidden_dim1", 256)
    hidden_dim2 = settings.get("hidden_dim2", 128)
    hidden_dim3 = settings.get("hidden_dim3", 64)
    latent_dim = settings.get("latent_dim", 32)
    ...
    mod1_encoder = Mod1Encoder(dims=[mod1_train_t.shape[0], 
                                     hidden_dim1, 
                                     hidden_dim2, 
                                     hidden_dim3],
    ...
    mod2_encoder = Mod1Encoder(dims=[mod2_train_t.shape[0], 
                                     hidden_dim1, 
                                     hidden_dim2, 
                                     hidden_dim3],
                               latent_dim=latent_dim,
...
def train_sCIN_unpaired(mod1_train: np.ndarray, 
                        mod2_train: np.ndarray, 
                        labels_train: List[np.ndarray], 
                        epochs: int, 
                        settings: Dict[str, Any],
                        is_pca_real: Optional[bool] = True,
                        is_pca_simulated: Optional[bool] = False, 
                        **kwargs) -> Dict[str, Any]:
    """
