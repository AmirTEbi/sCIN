...
class conAAE:
  def __init__(self,data1,data2,model_parameters):
    ...
    self.label_num=len(np.unique(self.RNA.tensors[1].cpu().numpy()))
    self.cell_num=self.RNA.tensors[0].shape[0]

    # FOR DEBUG
    self.is_trained = False
    
    
    self.netRNA=FC_Autoencoder(n_input=self.RNA.tensors[0].shape[1],n_hidden=self.RNA.tensors[0].shape[1],nz=self.args.nz)
    self.netATAC=FC_Autoencoder(n_input=self.ATAC.tensors[0].shape[1],n_hidden=self.ATAC.tensors[0].shape[1],nz=self.args.nz)
  ...
  def train_autoencoders(self,rna_inputs, atac_inputs, rna_class_labels=None, atac_class_labels=None):
    ...
    if self.args.MMD_loss:
      MMD_loss=self.mmd_rbf(atac_latents,rna_latents)
...
### main training loop
    for epoch in range(self.args.max_epochs):
        ...
        for idx, (rna_samples, atac_samples) in enumerate(zip(self.RNA_loader, self.ATAC_loader)):
            rna_inputs = rna_samples[0]
            atac_inputs = atac_samples[0]
            ...
            if self.args.augmentation:
              self.na_labels=torch.cat((rna_labels,rna_labels),0)
  ...
  def test(self,RNA_test,ATAC_test, save_dir=None, seed=None):
    ...
    ATAC_dim=ATAC_test.tensors[0].shape[1]
    RNA_dim=RNA_test.tensors[0].shape[1]
    label_num=len(np.unique(ATAC_test.tensors[1]))

    cell_num=ATAC_test.tensors[0].shape[0]
    ...
    for idx, (rna_samples,atac_samples) in enumerate(zip(RNA_loader,ATAC_loader)):
      ...
      rna_inputs = rna_inputs.cuda()
      atac_inputs = atac_inputs.cuda()
      self.netRNA.to(device='cuda')
      self.netATAC.to(device='cuda')
      rna_latents,_=self.netRNA(rna_inputs)
      atac_latents,_=self.netATAC(atac_inputs)
    ...
    #print(ATAC_label.shape)
    #pd.DataFrame(RNA_latent).to_csv('embedding/'+self.args.input_dir+f'/rna_emb{seed}.csv')
    #pd.DataFrame(ATAC_latent).to_csv('results/'+self.args.input_dir+f'/atac_emb{seed}.csv')
    ...
    #print(sum)
    print(type(RNA_latent), type(ATAC_latent))
