...
def compute_distance(mod1_embs:np.array, mod2_embs:np.array):
   
    num_cells = mod1_embs.shape[0]
...
def sort_match_cells(dist_matrix:np.array) -> tuple[np.array, np.array, np.array]:
   ...
   is_seen = np.zeros(num_cells, type=bool)
   ...
   sorted_indices_all = np.argsort(dist_matrix)
...
def assess(mod1_embs:np.array, mod2_embs:np.array, 
                         labels:np.array, seed:int=None) -> tuple[dict, int, float]:
